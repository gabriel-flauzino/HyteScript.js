...
module.exports = class Compiler {
    ...
    const types = {
      ...
      tag(c) {
        if (c === "(") {
          ...
          compiler.function.index =
            compiler.text.length + compiler.functions.length;
          compiler.opens++;
      ...
      function_parameters(c) {
        ...
        if (compiler.opens <= 0) {
          ...
          if (
            compiler.function.parameters[compiler.function.param] == undefined
          )
            compiler.function.parameters[compiler.function.param] = "";
    ...
    for (const func of compiler.functions) {
      ...
      for (const parameter of func.parameters) {
        if (["", undefined].includes(parameter)) newParameters.push(undefined);
          ...
          let compiledParameter = Compiler.compile(
  ...
  static async parse(d, compiledCode, returnResults = false) {
    ...
    if (d.clientOptions.debug === true && d.sourceCode == undefined)
      HscLog.debug(
        `parsing command: "${typeof d.command.name === "string" ? d.command.name : "unknown"}".\nCompiled code: ${require("util").inspect(compiled.nosource(), { showHidden: false, compact: true, depth: null, colors: true })}`,
      );

    if (d.sourceCode == undefined) d.sourceCode = compiled.source;

    for (const placeholder of d.data.placeholders) {
      compiled.text = compiled.text.map((text) =>
        text.replace(eval(`/${placeholder.name}/ig`), placeholder.value),
      );
    }
    ...
    for (const func of compiled.functions) {
        ...
        for (const parameter of func.parameters) {
          if (parameter == undefined) funcData.parameters.push(undefined);
            ...
            if (!loadedFunc.dontParse.includes(idx)) {
              ...
              if (parsed.result.toLowerCase() === "%blank%")
                funcData.parameters.push("");
              else
                funcData.parameters.push(
                  !loadedFunc.dontUnescape.includes(idx)
                    ? unescape(parsed.result)
                    : parsed.result,
                );
            } else {
      ...
      let result = await loadedFunc
        .run(d, ...d.function.parameters)
        .catch((e) => {
          if (d.data.logJSErrors) console.error(e);
          HscLog.error(
            `\x1b[31m#(${d.function.name}): (internal) ${e.message}`,
          );
        });
    ...
    d.data.message.content =
      compiled.text.join("").trim().replaceAll("\n", "") === ""
        ? undefined
        : unescape(compiled.text.join(""));
