...
module.exports = async (d) => {
  ...
  d.client.on("messageCreate", async (message) => {
    ...
    d.commandManager.default.forEach((commandData, commandName) => {
      if (commandData.ignorePrefix === true)
        ignoringPrefix.set(commandName, commandData);
    ...
    d.commandManager.alwaysExecute.forEach(async (commandData) => {
      ...
      if (!commandData.executeOnDM && message.channel.type === ChannelType.DM)
        return;
      ...
      if (
        JSON.stringify(parseCode.message.embeds) === "[]" &&
        JSON.stringify(parseCode.message.components) === "[]" &&
        parseCode.message.content == undefined
      )
        return;
    ...
    defaults.forEach(async (commandData, commandName) => {
      ...
      if (!commandData.executeOnDM && message.channel.type === ChannelType.DM)
        return;
      // checking prefix
      let triggeredPrefix = parsedPrefixes.find((prefix) =>
      ...
      if (
        commandName !== contentData.name.toLowerCase() &&
        !commandData.aliases.includes(contentData.name.toLowerCase())
      )
        return;
