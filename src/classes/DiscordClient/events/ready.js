...
module.exports = async (d) => {
  d.client.once("ready", async () => {
    ...
    if (d.status.length != 0) {
      // checking if d.status is not empty
      let index = 0;
    ...
    const res = await get("https://paebukoaapi.paebukoa.repl.co").catch((e) => {
      return { status: 0 };
    });
    ...
    if (d.clientOptions.debug === true)
      HscLog.debug(
        `\x1b[35;1m${d.functions.size || 0} functions \x1b[0mloaded.`,
      );
    ...
    HscLog.warn(
      `\x1b[31mYou're using a dev version, which means that it can contains serious bugs and stability problems.\nPlease, use v${res.data.hytescript.version} if you're looking for a stable version.\x1b[0m`,
    );
    HscLog.info(`\x1b[0mClient initialized on \x1b[36;1mv${version}\x1b[0m.`);
    if (
      typeof res.data.hytescript.ownerMessage === "string" &&
      res.data.hytescript.ownerMessage !== ""
    )
      HscLog.info(
        `\x1b[36m"${colorful(res.data.hytescript.ownerMessage, 82, 87)}\x1b[36m"\x1b[0m - paebukoa`,
      );
