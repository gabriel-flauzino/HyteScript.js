class Properties {
  ...
  static guild(guild, property) {
    const filteredProps = {
      ...
      usermemberscount:
        guild.members?.cache?.filter?.((x) => !x.bot)?.size || 0,
  ...
  static guildMember(guildMember, property) {
    const filteredProps = {
      ...
      stagevoicerequesttospeaktimestamp:
        guildMember.voice?.requestToSpeakTimestamp,
      device:
        guildMember.presence?.clientStatus?.desktop != undefined
          ? "desktop"
          : guildMember.presence?.clientStatus?.web != undefined
            ? "web"
            : guildMember.presence?.clientStatus?.mobile != undefined
              ? "mobile"
              : undefined,
