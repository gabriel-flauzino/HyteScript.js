...
module.exports = class Utils extends BaseUtils {
  ...
  static async parseMessage(d, message, removeEmpty = true) {
    ...
    if (parsedMessage.message.content == "" && removeEmpty)
      delete parsedMessage.message.content;
    if (parsedMessage.message.embeds[0] == undefined && removeEmpty)
      delete parsedMessage.message.embeds;
    if (parsedMessage.message.components[0] == undefined && removeEmpty)
      delete parsedMessage.message.components;
    if (parsedMessage.message.files[0] == undefined && removeEmpty)
      delete parsedMessage.message.files;
