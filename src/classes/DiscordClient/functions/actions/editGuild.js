  ...
  parameters: [
    {
      name: "Options",
      description:
        "The options for edit guild. Check #(editGuild) in HyteScript Docs for more details.",
      optional: "true",
    ...
    optionsData.functions = new Functions(optionsData.functions)
      ...
      .set("setAFKChannel", {
        async run(d, channelId = d.channel?.id) {
          ...
          if (!channel)
            return new d.error("invalid", d, "channel ID", channelId);
          if (channel.type !== ChannelType.GuildVoice)
            return new d.error(
              "custom",
              d,
              `provided channel "${channelId}" is not a voice channel`,
            );
        ...
        async run(d, channelId = d.channel?.id) {
          ...
          if (channel.type !== ChannelType.GuildText)
            return new d.error(
              "custom",
              d,
              `provided channel "${channelId}" is not a text channel`,
            );
      ...
      .set("setIcon", {
        async run(d, icon, isBuffer = "false") {
          ...
          if (isBuffer == "true") {
            ...
            if (buffer == undefined)
              return new d.error("invalid", d, "buffer name", icon);
      ...
      .set("setSystemChannelFlags", {
        async run(d, ...flags) {
          ...
          for (const flag of flagsTypes) {
            if (!(flag.toLowerCase() in flags))
              return new d.error("invalid", d, "flag", flag);
      ...
      .set("setBoostsProgressBar", {
        async run(d, enabled = "true") {
          if (enabled == undefined)
            return new d.error("required", d, "enabled");
      ...
      .set("setDescription", {
        async run(d, description) {
          if (description == undefined)
            return new d.error("required", d, "description");
