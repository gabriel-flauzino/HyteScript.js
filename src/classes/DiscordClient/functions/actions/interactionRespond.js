...
module.exports = {
  ...
  run: async (d, choices) => {
    ...
    if (d.interaction.responded)
      return new d.error(
        "custom",
        d,
        "that interaction already have been respoded",
      );

    const responseChoices = [];

    let choicesData = clone(d);
    choicesData.functions = new Functions(choicesData.functions).set(
      "setchoice",
      {
