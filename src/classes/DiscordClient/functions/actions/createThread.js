...
module.exports = {
  ...
  usage:
    "name | messageId? | type? | autoArchiveDuration? | invitable? | channelId? | guildId? | returnId?",
  parameters: [
    {
      name: "Name",
  ...
  run: async (
    ...
    let archiveDurationType =
      archiveDurationTypes[autoArchiveDuration.toLowerCase()];
    if (!archiveDurationType)
      return new d.error(
        "invalid",
        d,
    ...
    if (!channel.threads)
      return d.thowError.func(
        d,
        "the provided channel doesn't support threads",
      );

    let newThread = await channel.threads
      .create({
        name,
        type: threadType,
        message: messageId,
