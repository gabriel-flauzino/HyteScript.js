    ...
    actionRowData.functions = new Functions(actionRowData.functions)
      .set("addSelectMenu", {
        ...
        async run(
          ...
          if (placeholder == undefined)
            return new d.error("required", d, "placeholder");
          if (customId == undefined)
            return new d.error("required", d, "custom ID");
          if (options == undefined)
            return new d.error("required", d, "options");

          if (isNaN(min) || Number(min) < 1)
            return new d.error("invalid", d, "min values", min);
          if ((isNaN(max) || Number(max) < Number(min)) && max != undefined)
            return new d.error("invalid", d, "max values", max);
          ...
          optionsData.functions = new Functions(optionsData.functions).set(
            "addoption",
            {
              run: async (
                d,
                ...
                if (label == undefined)
                  return new d.error("required", d, "label");
                if (value == undefined)
                ...
                if (description != undefined)
                  selectMenuOption.setDescription(description);
      ...
      .set("addButton", {
        async run(d, style, label, customId, disabled = "false", emoji) {
          if (style == undefined)
            return new d.error("required", d, "style type");
          if (customId == undefined)
            return new d.error("required", d, "custom ID");
          ...
          if (style == undefined)
            return new d.error("invalid", d, "style", style);
