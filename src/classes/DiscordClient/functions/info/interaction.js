...
module.exports = {
  ...
  run: async (d, type, ...parameters) => {
    ...
    let types = {
      slashOption(name) {
        if (!["interaction", "commandInteraction"].includes(d.eventType))
          return new d.error(
            "notAllowed",
            d,
            "interaction or commandInteraction types",
          );
      ...
      selected(index = "all") {
        if (!["interaction", "selectMenuInteraction"].includes(d.eventType))
          return new d.error(
            "notAllowed",
            d,
            "interaction or selectMenuInteraction types",
          );

        if (
          (isNaN(index) || Number(index) < 1) &&
        ...
        if (typeof d.value === "string") return d.value;
          ...
          return d.value?.map?.((x) => escape(x))?.join?.(",");
      ...
      targetted(type, property = "id") {
        if (
          ...
          return new d.error(
            "notAllowed",
            d,
            "interaction, userContextMenuInteraction or messageContextMenuInteraction types",
          );

        if (!(type.toLowerCase() in d.target))
          return new d.error("invalid", d, "taget type", type);

        return getProperty(
          type.toLowerCase(),
          d.target[type.toLowerCase()],
          property,
        );
      },
      customId() {
        if (
          ...
          return new d.error(
            "notAllowed",
            d,
            "interaction, buttonInteraction, selectMenuInteraction or modalSubmitInteraction types",
          );

        return d.customId;
      },
      modalComponent(type, customId) {
        if (!["interaction", "modalSubmitInteraction"].includes(d.eventType))
          return new d.error(
            "notAllowed",
            d,
            "interaction or modalSubmitInteraction types",
          );
      ...
      focused(property) {
        if (!["interaction", "autocompleteInteraction"].includes(d.eventType))
          return new d.error(
            "notAllowed",
            d,
            "interaction or autocompleteInteraction types",
          );

        if (property == undefined)
          return new d.error("required", d, "property");
