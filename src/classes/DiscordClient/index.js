...
class DiscordClient {
  ...
  constructor(options) {
    ...
    if (typeof token !== "string")
      return new error("client", `invalid TOKEN in <HIDEN>`);
    if (intents === "all")
      return new error(
        "client",
        `"all" intents is deprecated. Use intents array instead`,
      );
    if (!Array.isArray(intents))
      return new error("client", `invalid INTENTS in "${intents}"`);
    if (typeof prefix !== "string" && !Array.isArray(prefix))
      return new error("client", `invalid PREFIX in "${prefix}"`);
    ...
    if (typeof prefix === "string") prefix = [prefix];
      ...
      return new error(
        "client",
        `prefixes must be string. Received ${typeof wrong} in "${wrong}"`,
      );
    ...
    for (const intent of intents) {
      let foundIntent = validIntents.find(
        (x) =>
          x.toLowerCase() ===
      ...
      if (!foundIntent || !["string", "number"].includes(typeof intent))
        return new error("client", `invalid intent in "${intent}"`);

      validatedIntents.push(
        isNaN(foundIntent)
          ? IntentsBitField.Flags[foundIntent]
          : Number(foundIntent),
      );
    }
        ...
        if (typeof commandReq == "object") {
          ...
          for (const command of commands) {
            ...
            this.data.commands.push(
              new Command(command, this.data.commandManager),
            );
          }
        } else {
          HscLog.warn(
            `folderReader: command path "${file.path}" couldn't be loaded.`,
          );
        }
      } catch (e) {
        if (this.data.clientOptions.logJSErrors) console.error(e);
        HscLog.warn(
          `folderReader: command path "${file.path}" couldn't be loaded: ${e.message}`,
        );
      }
    }
  ...
  addEvents(...events) {
    ...
    if (wrongEvent)
      return new error(
        "client",
        `client#addEvents: events must be string. Received ${typeof wrongEvent} in "${wrongEvent}"`,
      );
    for (const event of events) {
      if (event.toLowerCase() === "ready")
        HscLog.warn(`'ready' event already have been preadded.`);
      else {
        let runEvent = this.data.events.get(event.toLowerCase());
        if (!runEvent)
          return new error("client", `invalid event in "${event}"`);
  ...
  addStatus(...optionsArray) {
    ...
    for (const options of optionsArray) {
      ...
      if (typeof time != "number" && typeof time === "string") {
        if (!isNaN(time)) time = Number(time);
          ...
          if (parsedTime.error)
            return new error(
              "client",
              `invalid time in "${time}" at status ${d.status.length + 1}.`,
            );
          time = parsedTime.ms;
        }
      }
      ...
      if (time < 0)
        return new error(
          "client",
          `time must be above to 0 at status ${d.status.length + 1}.`,
        );
      ...
      if (types[type.toUpperCase()] == undefined)
        return new error(
          "client",
          `invalid activity type at status ${d.status.length + 1}.`,
        );
  ...
  addDatabase(name, entries, options = {}) {
    ...
    if (typeof entries !== "object" || !JSON.stringify(entries).startsWith("{"))
      throw new TypeError(`entries must be an object.`);
    if (this.data.databases.hasOwnProperty(name))
      throw new TypeError(`database with name "${name}" already exists.`);
  ...
  addFunctions(...functions) {
    for (const func of functions) {
      ...
      this.data.functions.set(name.toLowerCase(), {
        description,
        usage,
        parameters,
        aliases,
