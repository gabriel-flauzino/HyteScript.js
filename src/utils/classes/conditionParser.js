      ...
      for (const or of ors) {
        ...
        if (!isValidFullSymbol(parse.symbol)) {
          ...
          if (![""].includes(parse.one)) {
            if (parse.one.startsWith(" "))
              parse.one = parse.one.replace(" ", "");
            if (parse.one.endsWith(" "))
              parse.one = this.data.replaceLast(parse.one, " ", "");
          }
          if (![""].includes(parse.two)) {
            if (parse.two.startsWith(" "))
              parse.two = parse.two.replace(" ", "");
            if (parse.two.endsWith(" "))
              parse.two = this.data.replaceLast(parse.two, " ", "");
          }

          parse.one = !isNaN(parse.one)
            ? Number(parse.one)
            : `\`${parse.one}\``;
          parse.two = !isNaN(parse.two)
            ? Number(parse.two)
            : `\`${parse.two}\``;

          let parsedCondition = eval(
            `${parse.one} ${parse.symbol} ${parse.two}`,
          );
