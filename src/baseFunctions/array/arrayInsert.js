module.exports = {
  description:
    "Inserts elements in provided index, without removing other elements.",
  ...
  run: async (d, name, index, ...elements) => {
    ...
    if (!d.data.arrays[name])
      return new d.error("invalid", d, "array name", name);

    if (isNaN(index) || Number(index <= 0))
      return new d.error("invalid", d, "index number", index);

    let start = d.data.arrays[name].slice(0, Number(index) - 1);
    let end = d.data.arrays[name].slice(
      Number(index) - 1,
      d.data.arrays[name].length,
    );
