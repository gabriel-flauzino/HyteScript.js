...
module.exports = {
  ...
  parameters: [
    ...
    },
    {
      name: "Finally code",
      description:
        "Code to be runned when tested code don't throws an error (can be left empty, then value returned will be try code).",
      optional: "true",
      defaultValue: "none",
    },
  ],
  ...
  run: async (d, tryCode, catchCode, finallyCode) => {
    ...
    if (d.err && !d.data.break) {
      ...
      catchData.functions = new BaseFunctions(
        { replaceLast, getDirFiles, clone },
        catchData.functions,
      ).set("error", {
