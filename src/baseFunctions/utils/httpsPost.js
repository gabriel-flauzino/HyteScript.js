...
module.exports = {
  ...
  parameters: [
    ...
    },
    {
      name: "Body",
      description:
        "The body to be sent in POST request. Read #(httpsPost) in HyteScript wiki for detailed explanation.",
      optional: "true",
      defaultValue: "none",
    },
  ],
  ...
  run: async (d, link, property, body) => {
    ...
    if (typeof body === "object") {
      ...
      bodyData.functions = new BaseFunctions(
        { getDirFiles, replaceLast, clone },
        bodyData.functions,
      )
        .set("addbodyproperty", {
        ...
        .set("addbodypropertiesbyjson", {
          async run(d, obj) {
            ...
            if (!obj.startsWith("{"))
              return new d.error("invalid", d, "JSON", obj);
    ...
    let response = await post(link, body).catch(
      (e) => new d.error("custom", d, e.message),
    );
    if (d.err) return;
    ...
    if (property == undefined) result = response.data;
      ...
      try {
        ...
        return new d.error(
          "custom",
          d,
          `invalid property in "${property}": ${e.message}`,
        );
      }
    }
